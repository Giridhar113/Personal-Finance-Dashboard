...
const API_TRANSACTIONS_URL = "/transactions";
...
function setApiStatus(status) {
  if (!apiStatusLabel) return;
  apiStatusLabel.textContent = status;
  apiStatusLabel.dataset.status = status.toLowerCase().replace(/\s+/g, "-");
}

async function getTransactionsFromApi() {
  const response = await fetch(API_TRANSACTIONS_URL, { cache: "no-store" });
  if (!response.ok) throw new Error("Unable to load backend transactions");
  const transactions = await response.json();
  if (!Array.isArray(transactions)) throw new Error("Invalid backend transaction response");
...
async function createTransactionInApi(transaction) {
  ...
  if (!response.ok) throw new Error("Unable to save transaction to backend");
  return response.json();
}

async function deleteTransactionFromApi(id) {
  const response = await fetch(`${API_TRANSACTIONS_URL}/${encodeURIComponent(id)}`, {
    method: "DELETE"
  });
  if (!response.ok) throw new Error("Unable to delete transaction from backend");
  return response.json();
}
...
async function hydrateTransactionsFromApi() {
  if (apiHydrationStarted) return;
...
  const date = data.get("txDate");
  ...
  try {
    await createTransactionInApi(transaction);
    await getTransactionsFromApi();
  } catch {
    const transactions = loadTransactions();
...
transactionBody?.addEventListener("click", (event) => {
  ...
  const finishDelete = async () => {
    try {
      await deleteTransactionFromApi(removeId);
      await getTransactionsFromApi();
    } catch {
