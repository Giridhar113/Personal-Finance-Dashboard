...
const DEFAULT_TRANSACTIONS = [
  ...
  { id: "seed-2026-05-shopping", activity: "Shopping", category: "Other", date: "2026-05-21", amount: 1000, type: "debit", status: "Tracked" },
  ...
  { id: "seed-2026-04-shopping", activity: "Shopping", category: "Other", date: "2026-04-18", amount: 2500, type: "debit", status: "Tracked" },
...
function renderTransactionTables(transactions, stats) {
  ...
  if (transactionBody) {
    transactionBody.innerHTML = sorted.map(transactionRowTemplate).join("");
  }
...
function transactionRowTemplate(item) {
  ...
  return `
    <tr data-id="${item.id}">
      ...
      <td><button class="delete-tx-btn" type="button" data-id="${item.id}" aria-label="Delete ${item.activity}">Delete</button></td>
    </tr>
  `;
}
...
transactionBody?.addEventListener("click", (event) => {
  ...
  if (!button) return;
  const transactions = loadTransactions().filter(item => item.id !== button.dataset.id);
  saveTransactions(transactions);
  refreshDerivedData();
  renderHomeSnapshots();
  renderHomePeriodCards();
